<template>
    <div class="container">
        <h1>CREATE Thunder</h1>
        <div>
            <p>지역을 선택하세요</p>
            <select v-model="thunder.region">
        ...
        <div>
            <p>산을 선택하세요</p>
            <select v-model="thunder.tag" :disabled="!thunder.region">
                <option disabled value="">산 선택</option>
                <option
                    v-for="mountain in mountainData[thunder.region]"
                    :value="mountain"
                    :key="mountain"
                >
                    {{ mountain }}
        ...
        <div>
            ...
            <label for="input-title">
                <input type="text" name="input-title" v-model="thunder.title" />
            </label>
        </div>
        <div>
            <p>머리글을 입력하세요</p>
            <label for="input-header">
                <input type="textarea" name="input-header" v-model="thunder.header" />
            </label>
        </div>
        <div>
            <p>출발 날짜를 입력하세요</p>
        ...
        <div>
            ...
            <label for="input-end-date">
                <input
                    type="date"
                    name="input-end-date"
                    :min="thunder.startDate"
                    max="2024-12-31"
                    v-model="thunder.endDate"
                />
            </label>
        </div>
        <div>
        ...
        <editor-content :editor="editor" v-model="thunder.content" class="editor-content" />

        <div v-if="thunder.tag !== ''">
        ...
        <button @click="createThunder">글 작성</button>
    </div>
</template>
...
const createThunder = async () => {
    ...
    thunder.value.writer = storeUser.user.nickname;
    thunder.value.content = editor.value.getHTML();
    thunder.value.trail = JSON.stringify(hikingOrder.value);
    thunder.value.hikingTime = hikingTime.value;
    thunder.value.hikingDist = hikingDist.value;

    const formData = new FormData();

    formData.append('media', media.value);
    formData.append('thunder', JSON.stringify(thunder.value));

    await store.createThunder(formData);
