<template>
    <div class="container" :style="{ backgroundColor: containerBgColor }">
        ...
        <div :class="['menu', showMenu ? 'show' : '']">
            <button id="filter-all-place" class="menuItem" @click="clickAllPlaceButton('#c2c2c4')">All Place</button>
        ...
        <div class="tag-filter-nav">
            <button class="tag-nav-item" v-for="tag in uniqueTags" :key="tag" @click="store.selectBoardByTag(tag)">
                {{ tag }}
            </button>
        </div>
        ...
        <div class="post-card-container">
            <div class="post-card-element" v-for="board in store.boardList" :key="board.boardId">
                <RouterLink :to="`/board/${board.boardId}`">
                    <img :src="board.media" loading="lazy" />
                </RouterLink>
                <button class="board-tag" @click="store.selectBoardByTag(board.tag)">
                    {{ board.tag }}
                </button>
                <p class="board-title">{{ board.title }}</p>
                <p class="board-content">{{ board.header }}</p>
            </div>
        </div>
    </div>
</template>
...
const containerBgColor = ref('#c2c2c4');
...
const clickMainNavButton = (color, region) => {
    containerBgColor.value = color;
    store.selectBoardByRegion(region);
};

const uniqueTags = computed(() => {
    const tags = store.tagsByRegion.map((board) => board.tag);
